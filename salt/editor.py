...
class Editor:
    def __init__(self, onnx_model_path, dataset_path, categories=None, coco_json_path=None):
        ...
        # 获取图像数据，支持子文件夹结构
        result = self.dataset_explorer.get_image_data(self.image_id)
        if result[0] is None:  # 检查是否成功加载
            raise ValueError(f"Failed to load image data for image_id: {self.image_id}")
        
        self.image, self.image_embedding, annotations, image_name = result
        # 为了兼容性，创建image_bgr
    ...
    def next_image(self):
        # 在切换图像前，如果有未保存的掩码，自动保存
        if self.curr_inputs.curr_mask is not None:
            print(f"自动保存当前图像 {self.image_id} 的掩码标注")
            self.save_ann()
        ...
        result = self.dataset_explorer.get_image_data(self.image_id)
        if result[0] is None:  # 检查是否成功加载
            print(f"Warning: Failed to load image data for image_id: {self.image_id}, skipping...")
            return self.next_image()  # 递归尝试下一张图片
    ...
    def prev_image(self):
        ...
        if result[0] is None:  # 检查是否成功加载
            print(f"Warning: Failed to load image data for image_id: {self.image_id}, skipping...")
            return self.prev_image()  # 递归尝试上一张图片
